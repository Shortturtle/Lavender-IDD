...
export declare class CanvasRecorderPlugin extends AViewerPlugin<'start' | 'stop' | 'error' | 'encode-progress'> implements IConvergedCanvasRecorder {
    ...
    mimeType: 'auto' | 'image/png' | 'image/jpeg' | 'image/webp' | 'video/mp4' | string;
    ...
    /**
     * Prompts for duration if not specified
     * @param duration
     * @param ccw
     * @param angle
     * @param ease
     */
    download360Recording(duration?: number | null, ccw?: boolean, angle?: number, ease?: Easing): Promise<void>;
    private _turntableAnimationAngle;
    private _turntableAnimationDuration;
    private _turntableAnimationEasing;
    uiConfig: UiObjectConfig;
    recordCamera360(durationNum: number, ccw?: boolean, angle?: number, ease?: Easing): Promise<Blob | undefined>;
    protected _getRecorder(type: 'ffmpeg.js' | 'canvas-media' | 'image'): ICanvasRecorder<any>;
}
