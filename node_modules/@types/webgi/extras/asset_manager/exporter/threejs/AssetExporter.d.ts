import { IRenderTarget, ISceneObject, TAssetTypes } from 'webgi/interfaces';
import { AnyOptions, SimpleEventDispatcher } from 'ts-browser-helpers';
import { ExportFileOptions, IAssetExporter, IExporter } from 'webgi/extras/asset_manager/AssetManager';
import { ObjectProcessorMap } from 'webgi/helpers/ObjectProcessorMap';
import { ViewerApp } from 'webgi/viewer/ViewerApp';
export type BlobExt = Blob & {
    ext: string;
};
export declare class AssetExporter extends SimpleEventDispatcher<'exporterCreate' | 'exportFile'> implements IAssetExporter {
    private _processors;
    get processors(): ObjectProcessorMap<TAssetTypes>;
    readonly Exporters: IExporter[];
    getExporter(...ext: string[]): IExporter | undefined;
    constructor(viewer: ViewerApp, options?: AnyOptions);
    exportObject(obj?: ISceneObject | IRenderTarget, options?: ExportFileOptions): Promise<BlobExt | undefined>;
    private _exportFile;
    private _createParser;
    private _cachedParsers;
    private _getParser;
    processBeforeExport(obj: ISceneObject | IRenderTarget, options?: AnyOptions): Promise<{
        obj: any;
        ext: string;
        typeExt?: string;
    } | undefined>;
    dispose(): void;
}
//# sourceMappingURL=AssetExporter.d.ts.map